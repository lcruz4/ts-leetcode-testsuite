...
(async () => {
  ...
  for (const problemDir of problemDirs) {
    if (problem === problemDir) {
      ...
      const { default: solution } = await import(resolve(__dirname, problemDir, 'index.ts'));

      try {
        const { default: testFn } = await import(resolve(__dirname, problemDir, 'test.ts'));
        test = testFn;
      } catch (e) {}
      ...
      for (const file of pFiles) {
        if (file.startsWith('input' + (testInd || ''))) {
          ...
          const { default: input } = await import(resolve(__dirname, problemDir, file));
          const outputFilename = `output${inputInd}.json`;
          let output;

          try {
            const { default: outputJson } = await import(resolve(__dirname, problemDir, outputFilename));
            output = outputJson;
          } catch (e) {}
          ...
          if (!timeOnly) {
            if (output !== undefined) {
              ...
              if (!testResult) {
                log(`Answer: ${trunc(strAns)} is incorrect!\nExpected: ${trunc(strOutput)}\nSolution finished in ${(end - start)}ms`, 'red');
                return;
              }
            } else if (writeOutput) {
              writeFileSync(resolve(__dirname, problemDir, outputFilename), strAns);
            }
          }
          log(`Answer: ${trunc(strAns)}\nSolution finished in ${(end - start).toFixed(3)}ms`, 'green');
        }
      }
      return;
    }
