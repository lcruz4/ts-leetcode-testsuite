...
(async ()=> {
  const problemDirs = readdirSync(resolve(__dirname, '../leetcode')).filter(dir => dir.startsWith('p')).reverse();
  ...
  */
  ...
  const tests = readdirSync(resolve(__dirname, '../leetcode', problemDir)).filter(file => file.startsWith('input'));
  const choices = ['All', ...tests.reverse().map(test => test.match(/\d+/)?.[0])];
